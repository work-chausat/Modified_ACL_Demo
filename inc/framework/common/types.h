...
// Definition of the file header of the model file
struct ModelFileHeader {
  uint32_t magic = MODEL_FILE_MAGIC_NUM;               // magic number of DOMI 4
  uint32_t headsize = MODEL_FILE_HEAD_LEN;             // length of the model header. The value is fixed at 256 4
  uint32_t version = MODEL_VERSION;                    // version 1.0 4
  uint8_t checksum[MODEL_FILE_CHECKSUM_LENGTH] = {0U};  // signature 1*64=64
  uint32_t length = 0U;  // Ciphertext length. In the non-encryption model, the length is the plaintext length. 4
  // whether encrypted 0:not encrypt, 1:encrypt
  uint8_t is_encrypt = static_cast<uint8_t>(ModelEncryptType::UNENCRYPTED); // 1
  uint8_t is_checksum = static_cast<uint8_t>(ModelCheckType::CHECK);            // whether to check the checksum  1
  uint8_t modeltype = 0U;                                  // 0:IR model 1:standard model 2:OM Tiny model 3:flow model 1
  uint8_t genmode = 0U;                                    // 0：offline generate 1：online generate 1
  uint8_t name[MODEL_NAME_LENGTH] = {0U};                  // Model name, which contains 32 characters 32
  uint32_t ops = 0U;                                       // Computing power (Kops) 4
  uint8_t userdefineinfo[USER_DEFINE_INFO_LENGTH] = {0U};  // User-defined information. The value contains 32 characters 32
  uint32_t om_ir_version = 0U;                            // OM IR version number 4
  uint32_t model_num = 0U;                              // Number of subgraphs 4
  uint8_t platform_version[PLATFORM_VERSION_LEN] = {0U};  // Platform version 20
  uint8_t platform_type = {0U};                          // Platform type 1
  uint8_t padd[3] = {0};  // For initializing aligned memory 3
  uint64_t model_length = 0UL; // Model length 8
  uint8_t need_check_os_cpu_info = static_cast<uint8_t>(OsCpuInfoCheckTyep::NO_CHECK); // 1
  uint8_t reserved[MODEL_FILE_RESERVED_LENGTH] = {0U};  // Reserved field 64 -> 63
};
