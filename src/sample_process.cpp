...
Result SampleProcess::DemoProcess()
{
    Result ret;
    printf("\n------------------------------------\n");
    INFO_LOG("now encrypt sample and save...");
    ret = EncryptAndSave();
    if (ret != SUCCESS) {
        ERROR_LOG("execute EncryptAndSave failed");
        return FAILED;
    }

    printf("\n------------------------------------\n");
    INFO_LOG("now execute encrypted sample...");
    ret = DecryptAndProcess();
    if (ret != SUCCESS) {
        ERROR_LOG("execute DecryptAndProcess failed");
        return FAILED;
    }
...
Result SampleProcess::EncryptAndSave()
{
    ModelProcess modelProcess;
    const char* omModelPath = "../model/resnet50.om";
    Result ret = modelProcess.EncryptModelWeightAndSave(omModelPath);
    if (ret != SUCCESS) {
        ERROR_LOG("execute EncryptModelWeight failed");
        return FAILED;
    }
    return SUCCESS;
}

Result SampleProcess::DecryptAndProcess()
{
    // model init
    ModelProcess modelProcess;
    const char* omModelPath = "../model/resnet50_encrypted.om";
    Result ret = modelProcess.LoadEncryptedModel(omModelPath);
    if (ret != SUCCESS) {
        ERROR_LOG("execute LoadModel failed");
        return FAILED;
    }
    ...
    for (size_t index = 0; index < sizeof(testFile) / sizeof(testFile[0]); ++index) {
        ...
        // 在执行模型之前，decrypt model weight
        ret = modelProcess.DecryptModelWeight();
        if (ret != SUCCESS) {
            ERROR_LOG("execute DecryptModelWeight failed");
            aclrtFree(picDevBuffer);
            return FAILED;
        }
